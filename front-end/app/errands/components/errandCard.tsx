...
export default function ErrandCard(props:{errand:Errands,varient:Errandvarients}){
    ...
    return(
        <motion.div whileHover={{y:-15}} whileTap={{scale:0.9}} className="flex flex-col shadow-md rounded w-full h-64">
            <Link href={"/errands/"+errand.errand_id}>
                ...
                <p className={`text-black mx-3 mt-2 ${varient=='ExploreCard'?"line-clamp-2":"line-clamp-1"}`}>{errand.errand_title}</p>
                <p className="text-slate-400 mx-3 line-clamp-2 h-12 ">{errand.errand_desc}</p>
                <div className="flex justify-between"><span className="flex ml-3 mt-2">$<p className="text-teal-500 mx-1">{errand.pay}</p></span><p className={`mt-2 mr-3 ${varient=='ExploreCard'?"line-clamp-2":"line-clamp-1"}`}>{errand.auth_name}</p></div>
            </Link>
        </motion.div>
    )
}
